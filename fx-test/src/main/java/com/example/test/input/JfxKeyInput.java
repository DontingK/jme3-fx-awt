...
public class JfxKeyInput extends JfxInput implements KeyInput {
    ...
    public JfxKeyInput(Application application) {
        super(application);
    ...
    public void bind(Node node) {
        super.bind(node);
        node.setOnKeyPressed( this::processKeyPressed);
        node.setOnKeyReleased( this::processKeyReleased);
    }
    ...
    private void onKeyEvent(KeyEvent keyEvent, boolean pressed) {
        ...
        KeyInputEvent event = new KeyInputEvent(code, keyChar, pressed, repeating);
        event.setTime(getInputTimeNanos());

        application.enqueue((Runnable) () -> keyInputEvents.add(event));
//        EditorContent.runJme(() -> keyInputEvents.add(event));
    }
